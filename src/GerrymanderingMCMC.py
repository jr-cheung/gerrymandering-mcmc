...
class GerrymanderingMCMC():
        ...
        while cuttable is False:
            ...
            for edge in mst_combined_subgraph.edges:
                ...
                if (attempt_count == 10000):
                    print("WARNING: Failed to make a recom after > 10000 iterations") if self.verbose else None
                    return graph
                else:
                    attempt_count += 1
                    (tail, head) = edge
    ...
    def process_graph(self, graph, data_obj):
        # State Constants
        num_districts = 9
        ...
        for i in range(num_districts):
            ...
            districting["districts"].append(district_json)

        # Form Districts
        enacted_polygons = [[] for i in range(num_districts)] if self.enacted_geometries == [] else None
        district_polygons = [[] for i in range(num_districts)]
        nodes = graph['nodes']
        for n in nodes:
            i = string.ascii_uppercase.index(n["district"])

            for k in n.keys():
                if k == "population":
                    districting["districts"][i]["totalPop"] += n[k]
                elif k == "id":
                    districting["districts"][i]["precincts"].append(int(n[k]))
                elif k == "geometry":
                    polygon = shape(n[k])
                    district_polygons[i].append(polygon)

                    if self.enacted_geometries == []:
                        i_enacted = string.ascii_uppercase.index(self.graph_json[str(n["id"])]["district"])
                        enacted_polygons[i_enacted].append(polygon)
                elif k == "voting_history" or k == "district" or k == "geometry":
                    pass
                else:
                    districting["districts"][i][k] += n[k]

        districts = districting["districts"]

        # Sum District Populations
        districting["totalPop"] = 0
        districting["totalVap"] = 0
        districting["whitePop"] = 0
        districting["whiteVap"] = 0
        districting["blackPop"] = 0
        districting["blackVap"] = 0
        districting["asianPop"] = 0
        districting["asianVap"] = 0
        districting["hispanicPop"] = 0
        districting["hispanicVap"] = 0
        for d in districts:
            districting["totalPop"] += d["totalPop"]
            districting["totalVap"] += d["totalVap"]
            districting["whitePop"] += d["whitePop"]
            districting["whiteVap"] += d["whiteVap"]
            districting["blackPop"] += d["blackPop"]
            districting["blackVap"] += d["blackVap"]
            districting["asianPop"] += d["asianPop"]
            districting["asianVap"] += d["asianVap"]
            districting["hispanicPop"] += d["hispanicPop"]
            districting["hispanicVap"] += d["hispanicVap"]
        # Graph Compactness
        # for i in range(num_districts):
        #     precincts = districts[i]["precincts"]
        #     perimeter_nodes = 0
        #
        #     for p in precincts:
        #         adj_list = graph["adjacency"][p-1]
        #         is_perimeter_node = True
        #         for neighbor in adj_list:
        #             if int(neighbor["id"]) not in precincts:
        #                 is_perimeter_node = False
        #                 break
        #
        #         if is_perimeter_node:
        #             perimeter_nodes += 1
        #
        #     if perimeter_nodes != 0:
        #         graph_compactness = len(precincts)/(perimeter_nodes**2)
        #         districts[i]["graphCompactness"] = graph_compactness

        # District Geometry and Polsby-Popper Compactness
        # self.enacted_geometries = []
        current_geometries = []
        compactness_sum = 0
        for i in range(num_districts):
            # print("Union")
            current_geometry = unary_union(district_polygons[i])
            current_geometries.append(current_geometry)
            if len(self.enacted_geometries) < num_districts:
                self.enacted_geometries.append(unary_union(enacted_polygons[i]))
            # print("Union finished")

            # districting["districts"][i]["geometry"] = mapping(current_geometry)
            # district_map = mapping(current_geometry)
            # fname = "output/district" + str(i+1) + ".json"
            # with open(fname, "w") as file:
            #     file.write(json.dumps(district_map))

            # print("Area Projection")
            district_area = transform(
                partial(
                    pyproj.transform,
                    pyproj.Proj(init="EPSG:4326"),
                    pyproj.Proj(
                        proj='aea',
                        lat_1=current_geometry.bounds[1],
                        lat_2=current_geometry.bounds[3])),
                current_geometry)
            # print("Area Projection finished")

            area_conversion_factor = 2589988.1103
            perimeter_conversion_factor = 1609.344

            area = district_area.area/area_conversion_factor
            perimeter = district_area.length/perimeter_conversion_factor
            polsby_popper = (4 * math.pi * area)/(perimeter**2)

            districting["districts"][i]["area"] = area
            districting["districts"][i]["perimeter"] = perimeter
            districting["districts"][i]["polsbyPopperCompactness"] = polsby_popper
            compactness_sum += polsby_popper

        districting["polsbyPopperCompactness"] = compactness_sum/num_districts

        # Gills Construct
        # print("Gills Construct")
        district_mapping = []
        for i in range(num_districts):
            current = current_geometries[i]
            max_overlap = -1
            max_i = -1
            for j in range(len(self.enacted_geometries)):
                if j in district_mapping:
                    continue

                overlap = current.intersection(self.enacted_geometries[j])
                if overlap.area > max_overlap:
                    max_overlap = overlap.area
                    max_i = j

            # District mapping
            district_mapping.append(max_i)
        # print("Gills Construct Mapping")

        # Swap districts
        ordered_districts = [districts[i] for i in district_mapping]
        districting["districts"] = ordered_districts
        districts = districting["districts"]

        for i in range(num_districts):
            districts[i]["number"] = i+1

        # print("Gills Construct Finished")

        # Equal Population
        pop_list = [d["totalPop"] for d in districting["districts"]]
        min_pop = min(pop_list)
        max_pop = max(pop_list)
        total_pop = districting["totalPop"]
        districting["equalPopulation"] = (max_pop-min_pop)/(total_pop/num_districts)

        # Equal Vap
        vap_list = [d["totalVap"] for d in districting["districts"]]
        min_vap = min(vap_list)
        max_vap = max(vap_list)
        total_vap = districting["totalVap"]
        districting["equalVap"] = (max_vap-min_vap)/(total_vap/num_districts)

        # Efficiency Gap ("eg"), and others
        for k in data_obj.keys():
            districting[k] = data_obj[k]

        return districting

    def insert_into_database(self, districting):
        statement = "INSERT INTO ArizonaDistrictings (compactness, demographicPopulation, equalPopulation, efficiencyGap, equalVap) " \
                    "VALUES (%s, %s, %s, %s, %s)"

        # with open('arizona_districting_test.json') as json_file:
        #     data = json.load(json_file)

        # Populate ArizonaDistrictings table
        demographicPop = {}
        for stat in districting:
            if stat not in ['districts', 'polsbyPopperCompactness', 'equalPopulation', 'eg', "equalVap"]:
                demographicPop[stat] = int(districting[stat])
        demographicPop = json.dumps(demographicPop)

        pols_pop = float(districting['polsbyPopperCompactness'])
        eq_pop = float(districting['equalPopulation'])
        eff_gap = float(districting['eg'])
        eq_vap = float(districting['equalVap'])
        ...
        values = (pols_pop, demographicPop, eq_pop, eff_gap, eq_vap)
        try:
        ...
        # Populate ArizonaDistrics
        values_list = []
        statement = 'INSERT INTO ArizonaDistricts (districtingId, number, compactness, demographicPopulation, perimeter, area, precincts) ' \
                    'VALUES (%s, %s, %s, %s, %s, %s, %s)'

        districtingId = int(cursor.lastrowid)

        for district in districting['districts']:
            demographicPopDistrict = {}
            for stat in district:
                if stat not in ['precincts', 'area', 'perimeter', 'polsbyPopperCompactness']:
                    demographicPopDistrict[stat] = int(district[stat])
            demographicPopDistrict = json.dumps(demographicPopDistrict)

            precincts = str(district['precincts'])
            pols_pop = float(district['polsbyPopperCompactness'])
            # geo = json.dumps(district['geometry'])
            perimeter = float(district['perimeter'])
            area = float(district['area'])
            number = int(district['number'])
            values_list.append((districtingId, number, pols_pop, demographicPopDistrict, perimeter, area, precincts))

        try:
            # Executing the SQL command
            cursor.executemany(statement, values_list)
    ...
    def generate_alternative_plans(self, rounds):
        ...
        for i in range(0, rounds):
            ...
            data_obj = self.__record_key_stats(graph)
            graph_json = json_graph.adjacency_data(graph)
            districting = self.process_graph(graph_json, data_obj)

            # fname = "output/recombination_of_districts_" + str(rounds) + "_" + str(self.count) + ".json"
            # self.__drawGraph(graph)
            # with open(fname, "w") as file:
            #     file.write(json.dumps(json_graph.adjacency_data(graph)))
            self.insert_into_database(districting)

            # fname = "output/arizona_districting_" + str(uuid.uuid4()) + ".json"
            # with open(fname, "w") as file:
            #     file.write(json.dumps(districting))
